...
1. `/family/create` 진입
2. (선택) 프로필 미완성 시 Step 1에서 내 프로필 정보 입력
3. Step 2: 가족 이름/설명 입력
4. Step 3: 가족 내 표시명/역할 입력
5. 생성 완료 → `/profile/:userId` 이동
...
1. 피드(`/`) 상단 가족 탭에서 가족 선택
2. 현재 가족 컨텍스트 변경 (Zustand)
3. 선택된 가족 기준으로 게시글/멤버 정보 표시

### E. 멤버 초대 (관리자)

1. `/family/:familyId/invite` 진입
2. 초대 코드 복사 또는 Web Share API 공유
3. 관리자만 새 코드 생성 가능, 생성 시 기존 코드 무효화
...
- `/family/create` → `create-family-page.tsx`
  - 프로필(선택) → 가족 정보 → 가족 내 프로필 설정
- `/family/join` → `join-family-page.tsx`
  - 초대 코드 입력 → 가족 정보 확인 → 프로필 설정 → `/` 이동
